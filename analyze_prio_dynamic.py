#!/usr/bin/env python3
"""
Script para analisar os resultados do algoritmo PRIO_DYNAMIC
...
def load_prio_dynamic_data():
    """Carrega os dados TME do algoritmo PRIO_DYNAMIC"""
    prio_dynamic_data = []
    data_dir = Path("data_PRIO_DYNAMIC")
    ...
    return pd.DataFrame(prio_dynamic_data)

def plot_prio_dynamic_analysis(df):
    """Gera gráficos de análise para o algoritmo PRIO_DYNAMIC"""
    
    # Criar diretório de saída se não existir
    output_dir = Path("data_PRIO_DYNAMIC")
    ...
    plt.errorbar(df['Processos'], df['Media'], 
                ...
                color='#17becf', alpha=0.7, label='±1 Desvio Padrão')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Análise do TME - Algoritmo PRIO_DYNAMIC', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_dynamic_tme_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Variancia'], 
            ...
            color='#bcbd22', label='Variância')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Variância do TME', fontsize=14)
    plt.title('Variância do TME - Algoritmo PRIO_DYNAMIC', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_dynamic_variance_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], range_values, 
            ...
            color='#7f7f7f', label='Range (Máx - Mín)')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Range do TME', fontsize=14)
    plt.title('Range do TME - Algoritmo PRIO_DYNAMIC', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_dynamic_range_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Minimo'], 
            ...
            color='#bcbd22', label='Mínimo', alpha=0.8)
    plt.plot(df['Processos'], df['Media'], 
            marker='o', linewidth=3, markersize=10, 
            color='#17becf', label='Médio', alpha=0.9)
    plt.plot(df['Processos'], df['Maximo'], 
            marker='^', linewidth=2, markersize=8, 
            color='#7f7f7f', label='Máximo', alpha=0.8)
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Comparação Mínimo/Médio/Máximo - Algoritmo PRIO_DYNAMIC', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_dynamic_min_med_max_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()

def create_prio_dynamic_summary(df):
    """Cria um resumo estatístico do algoritmo PRIO_DYNAMIC"""
    print("\n" + "="*80)
    print("RESUMO ESTATÍSTICO - ALGORITMO PRIO_DYNAMIC")
    print("="*80)
    ...
    # Salvar resumo em CSV dentro da pasta data_PRIO_DYNAMIC
    output_dir = Path("data_PRIO_DYNAMIC")
    output_dir.mkdir(exist_ok=True)
    df.to_csv(output_dir / 'prio_dynamic_summary.csv', index=False)
    print(f"\nResumo salvo em 'data_PRIO_DYNAMIC/prio_dynamic_summary.csv'")

def main():
    """Função principal"""
    print("Analisando dados do algoritmo PRIO_DYNAMIC...")
    
    # Carregar dados
    df = load_prio_dynamic_data()
    ...
    # Criar gráficos
    print("\nGerando gráficos...")
    plot_prio_dynamic_analysis(df)
    
    # Criar resumo
    create_prio_dynamic_summary(df)
    
    print("\nAnálise concluída!")
    print("Arquivos gerados em 'data_PRIO_DYNAMIC/':")
    print("- prio_dynamic_tme_analysis.png")
    print("- prio_dynamic_variance_analysis.png")
    print("- prio_dynamic_range_analysis.png")
    print("- prio_dynamic_min_med_max_comparison.png")
    print("- prio_dynamic_summary.csv")
