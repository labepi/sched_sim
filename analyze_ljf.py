#!/usr/bin/env python3
"""
Script para analisar os resultados do algoritmo LJF
...
def load_ljf_data():
    """Carrega os dados TME do algoritmo LJF"""
    ljf_data = []
    data_dir = Path("data_LJF")
    ...
    return pd.DataFrame(ljf_data)

def plot_ljf_analysis(df):
    """Gera gráficos de análise para o algoritmo LJF"""
    
    # Criar diretório de saída se não existir
    output_dir = Path("data_LJF")
    ...
    plt.errorbar(df['Processos'], df['Media'], 
                ...
                color='#2ca02c', alpha=0.7, label='±1 Desvio Padrão')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Análise do TME - Algoritmo LJF', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'ljf_tme_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Variancia'], 
            ...
            color='#8c564b', label='Variância')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Variância do TME', fontsize=14)
    plt.title('Variância do TME - Algoritmo LJF', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'ljf_variance_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], range_values, 
            ...
            color='#e377c2', label='Range (Máx - Mín)')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Range do TME', fontsize=14)
    plt.title('Range do TME - Algoritmo LJF', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'ljf_range_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Minimo'], 
            ...
            color='#8c564b', label='Mínimo', alpha=0.8)
    plt.plot(df['Processos'], df['Media'], 
            marker='o', linewidth=3, markersize=10, 
            color='#2ca02c', label='Médio', alpha=0.9)
    plt.plot(df['Processos'], df['Maximo'], 
            marker='^', linewidth=2, markersize=8, 
            color='#e377c2', label='Máximo', alpha=0.8)
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Comparação Mínimo/Médio/Máximo - Algoritmo LJF', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'ljf_min_med_max_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()

def create_ljf_summary(df):
    """Cria um resumo estatístico do algoritmo LJF"""
    print("\n" + "="*80)
    print("RESUMO ESTATÍSTICO - ALGORITMO LJF")
    print("="*80)
    ...
    # Salvar resumo em CSV dentro da pasta data_LJF
    output_dir = Path("data_LJF")
    output_dir.mkdir(exist_ok=True)
    df.to_csv(output_dir / 'ljf_summary.csv', index=False)
    print(f"\nResumo salvo em 'data_LJF/ljf_summary.csv'")

def main():
    """Função principal"""
    print("Analisando dados do algoritmo LJF...")
    ...
    # Criar gráficos
    print("\nGerando gráficos...")
    plot_ljf_analysis(df)
    
    # Criar resumo
    create_ljf_summary(df)
    
    print("\nAnálise concluída!")
    print("Arquivos gerados em 'data_LJF/':")
    print("- ljf_tme_analysis.png")
    print("- ljf_variance_analysis.png")
    print("- ljf_range_analysis.png")
    print("- ljf_min_med_max_comparison.png")
    print("- ljf_summary.csv")
