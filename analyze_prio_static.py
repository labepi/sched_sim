#!/usr/bin/env python3
"""
Script para analisar os resultados do algoritmo PRIO_STATIC
...
def load_prio_static_data():
    """Carrega os dados TME do algoritmo PRIO_STATIC"""
    prio_static_data = []
    data_dir = Path("data_PRIO_STATIC")
    ...
    return pd.DataFrame(prio_static_data)

def plot_prio_static_analysis(df):
    """Gera gráficos de análise para o algoritmo PRIO_STATIC"""
    
    # Criar diretório de saída se não existir
    output_dir = Path("data_PRIO_STATIC")
    ...
    plt.errorbar(df['Processos'], df['Media'], 
                ...
                color='#98df8a', alpha=0.7, label='±1 Desvio Padrão')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Análise do TME - Algoritmo PRIO_STATIC', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_static_tme_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title('Variância do TME - Algoritmo PRIO_STATIC', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_static_variance_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title('Range do TME - Algoritmo PRIO_STATIC', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_static_range_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Media'], 
            marker='o', linewidth=3, markersize=10, 
            color='#98df8a', label='Médio', alpha=0.9)
    ...
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Comparação Mínimo/Médio/Máximo - Algoritmo PRIO_STATIC', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_static_min_med_max_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()

def create_prio_static_summary(df):
    """Cria um resumo estatístico do algoritmo PRIO_STATIC"""
    print("\n" + "="*80)
    print("RESUMO ESTATÍSTICO - ALGORITMO PRIO_STATIC")
    print("="*80)
    ...
    # Salvar resumo em CSV dentro da pasta data_PRIO_STATIC
    output_dir = Path("data_PRIO_STATIC")
    output_dir.mkdir(exist_ok=True)
    df.to_csv(output_dir / 'prio_static_summary.csv', index=False)
    print(f"\nResumo salvo em 'data_PRIO_STATIC/prio_static_summary.csv'")

def main():
    """Função principal"""
    print("Analisando dados do algoritmo PRIO_STATIC...")
    
    # Carregar dados
    df = load_prio_static_data()
    ...
    # Criar gráficos
    print("\nGerando gráficos...")
    plot_prio_static_analysis(df)
    
    # Criar resumo
    create_prio_static_summary(df)
    
    print("\nAnálise concluída!")
    print("Arquivos gerados em 'data_PRIO_STATIC/':")
    print("- prio_static_tme_analysis.png")
    print("- prio_static_variance_analysis.png")
    print("- prio_static_range_analysis.png")
    print("- prio_static_min_med_max_comparison.png")
    print("- prio_static_summary.csv")
