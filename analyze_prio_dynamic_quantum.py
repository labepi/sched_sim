#!/usr/bin/env python3
"""
Script para analisar os resultados do algoritmo PRIO_DYNAMIC_QUANTUM
...
def load_prio_dynamic_quantum_data():
    """Carrega os dados TME do algoritmo PRIO_DYNAMIC_QUANTUM"""
    prio_dynamic_quantum_data = []
    data_dir = Path("data_PRIO_DYNAMIC_QUANTUM")
    ...
    for num_processos in sorted(process_groups.keys()):
        ...
        prio_dynamic_quantum_data.append({
    ...
    return pd.DataFrame(prio_dynamic_quantum_data)

def plot_prio_dynamic_quantum_analysis(df):
    """Gera gráficos de análise para o algoritmo PRIO_DYNAMIC_QUANTUM"""
    
    # Criar diretório de saída se não existir
    output_dir = Path("data_PRIO_DYNAMIC_QUANTUM")
    ...
    plt.errorbar(df['Processos'], df['Media'], 
                ...
                color='#ff9896', alpha=0.7, label='±1 Desvio Padrão')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Análise do TME - Algoritmo PRIO_DYNAMIC_QUANTUM', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_dynamic_quantum_tme_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Variancia'], 
            ...
            color='#c5b0d5', label='Variância')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Variância do TME', fontsize=14)
    plt.title('Variância do TME - Algoritmo PRIO_DYNAMIC_QUANTUM', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_dynamic_quantum_variance_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], range_values, 
            ...
            color='#fdb462', label='Range (Máx - Mín)')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Range do TME', fontsize=14)
    plt.title('Range do TME - Algoritmo PRIO_DYNAMIC_QUANTUM', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_dynamic_quantum_range_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Minimo'], 
            ...
            color='#c5b0d5', label='Mínimo', alpha=0.8)
    plt.plot(df['Processos'], df['Media'], 
            marker='o', linewidth=3, markersize=10, 
            color='#ff9896', label='Médio', alpha=0.9)
    plt.plot(df['Processos'], df['Maximo'], 
            marker='^', linewidth=2, markersize=8, 
            color='#fdb462', label='Máximo', alpha=0.8)
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Comparação Mínimo/Médio/Máximo - Algoritmo PRIO_DYNAMIC_QUANTUM', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'prio_dynamic_quantum_min_med_max_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()

def create_prio_dynamic_quantum_summary(df):
    """Cria um resumo estatístico do algoritmo PRIO_DYNAMIC_QUANTUM"""
    print("\n" + "="*80)
    print("RESUMO ESTATÍSTICO - ALGORITMO PRIO_DYNAMIC_QUANTUM")
    print("="*80)
    ...
    # Salvar resumo em CSV dentro da pasta data_PRIO_DYNAMIC_QUANTUM
    output_dir = Path("data_PRIO_DYNAMIC_QUANTUM")
    output_dir.mkdir(exist_ok=True)
    df.to_csv(output_dir / 'prio_dynamic_quantum_summary.csv', index=False)
    print(f"\nResumo salvo em 'data_PRIO_DYNAMIC_QUANTUM/prio_dynamic_quantum_summary.csv'")

def main():
    """Função principal"""
    print("Analisando dados do algoritmo PRIO_DYNAMIC_QUANTUM...")
    
    # Carregar dados
    df = load_prio_dynamic_quantum_data()
    ...
    # Criar gráficos
    print("\nGerando gráficos...")
    plot_prio_dynamic_quantum_analysis(df)
    
    # Criar resumo
    create_prio_dynamic_quantum_summary(df)
    
    print("\nAnálise concluída!")
    print("Arquivos gerados em 'data_PRIO_DYNAMIC_QUANTUM/':")
    print("- prio_dynamic_quantum_tme_analysis.png")
    print("- prio_dynamic_quantum_variance_analysis.png")
    print("- prio_dynamic_quantum_range_analysis.png")
    print("- prio_dynamic_quantum_min_med_max_comparison.png")
    print("- prio_dynamic_quantum_summary.csv")
