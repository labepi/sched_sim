...
struct proc * scheduler(struct proc * current) {
    ...
    // SJF: Seleciona o processo com menor remaining_time > 0
    int min_index = -1;
    int min_time = -1;
    int len = length(ready);
    for (int i = 0; i < len; i++) {
        struct proc * p = get(ready, i);
        if (p->remaining_time > 0 && (min_index == -1 || p->remaining_time < min_time)) {
            min_index = i;
    ...
    if (min_index == -1) {
        // Nenhum processo com tempo restante válido
        return NULL;
    }
