#!/usr/bin/env python3
"""
Script para analisar os resultados do algoritmo FIFO
...
def load_fifo_data():
    """Carrega os dados TME do algoritmo FIFO"""
    fifo_data = []
    data_dir = Path("data_FIFO")
    ...
    return pd.DataFrame(fifo_data)

def plot_fifo_analysis(df):
    """Gera gráficos de análise para o algoritmo FIFO"""
    
    # Criar diretório de saída se não existir
    output_dir = Path("data_FIFO")
    ...
    plt.errorbar(df['Processos'], df['Media'], 
                ...
                color='#1f77b4', alpha=0.7, label='±1 Desvio Padrão')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Análise do TME - Algoritmo FIFO', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'fifo_tme_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Variancia'], 
            ...
            color='#ff7f0e', label='Variância')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Variância do TME', fontsize=14)
    plt.title('Variância do TME - Algoritmo FIFO', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'fifo_variance_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], range_values, 
            ...
            color='#2ca02c', label='Range (Máx - Mín)')
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Range do TME', fontsize=14)
    plt.title('Range do TME - Algoritmo FIFO', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'fifo_range_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.plot(df['Processos'], df['Media'], 
            marker='o', linewidth=3, markersize=10, 
            color='#1f77b4', label='Médio', alpha=0.9)
    plt.plot(df['Processos'], df['Maximo'], 
            marker='^', linewidth=2, markersize=8, 
            color='#2ca02c', label='Máximo', alpha=0.8)
    
    plt.xlabel('Número de Processos', fontsize=14)
    plt.ylabel('Tempo Médio de Espera (TME)', fontsize=14)
    plt.title('Comparação Mínimo/Médio/Máximo - Algoritmo FIFO', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'fifo_min_med_max_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()

def create_fifo_summary(df):
    """Cria um resumo estatístico do algoritmo FIFO"""
    print("\n" + "="*80)
    print("RESUMO ESTATÍSTICO - ALGORITMO FIFO")
    print("="*80)
    ...
    # Salvar resumo em CSV dentro da pasta data_FIFO
    output_dir = Path("data_FIFO")
    output_dir.mkdir(exist_ok=True)
    df.to_csv(output_dir / 'fifo_summary.csv', index=False)
    print(f"\nResumo salvo em 'data_FIFO/fifo_summary.csv'")

def main():
    """Função principal"""
    print("Analisando dados do algoritmo FIFO...")
    ...
    # Criar gráficos
    print("\nGerando gráficos...")
    plot_fifo_analysis(df)
    
    # Criar resumo
    create_fifo_summary(df)
    
    print("\nAnálise concluída!")
    print("Arquivos gerados em 'data_FIFO/':")
    print("- fifo_tme_analysis.png")
    print("- fifo_variance_analysis.png")
    print("- fifo_range_analysis.png")
    print("- fifo_min_med_max_comparison.png")
    print("- fifo_summary.csv")
